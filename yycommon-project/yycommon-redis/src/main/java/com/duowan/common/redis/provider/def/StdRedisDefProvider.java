 ...
 * @author Arvin
 */
public class StdRedisDefProvider implements RedisDefProvider {
    @Override
    public boolean support(RedisDef redisDef) {
        return null != redisDef && redisDef instanceof StdRedisDef;
    }
    ...
    public boolean checkAndApplyDefaultConfig(RedisDef redisDef) {
        ...
        StdRedisDef stdRedisDef = (StdRedisDef) redisDef;
        ...
        AssertUtil.assertNotBlank(stdRedisDef.getId(), "RedisDef.id 不能为空");
        AssertUtil.assertNotBlank(stdRedisDef.getServer(), "[" + stdRedisDef.getId() + "]RedisDef.server不能为空");
     ...
     * 应用默认的连接池配置
     *
     * @param stdRedisDef RedisDef 定义
     * @return 返回设置了默认连接池之后的配置
     */
    private Map<String, String> applyDefaultPoolConfig(StdRedisDef stdRedisDef) {
