 ...
 * @author Arvin
 */
public class SentinelRedisDefProvider implements RedisDefProvider {
    @Override
    public boolean support(RedisDef redisDef) {
        return null != redisDef && redisDef instanceof SentinelRedisDef;
    }

    @Override
    public boolean checkAndApplyDefaultConfig(RedisDef redisDef) {
        if (!support(redisDef)) {
            return false;
        }

        SentinelRedisDef sentinelRedisDef = (SentinelRedisDef) redisDef;

        sentinelRedisDef.setDatabase(filterDefaultDatabase(sentinelRedisDef.getDatabase(), "0"));
        sentinelRedisDef.setPassword(filterDefaultPassword(sentinelRedisDef.getPassword(), null));
        sentinelRedisDef.setTimeout(filterDefaultTimeout(sentinelRedisDef.getTimeout(), "3000"));
        sentinelRedisDef.setPoolConfig(applyDefaultPoolConfig(sentinelRedisDef));

        AssertUtil.assertNotBlank(sentinelRedisDef.getId(), "RedisDef.id 不能为空");
        AssertUtil.assertNotBlank(sentinelRedisDef.getSentinels(), "[" + sentinelRedisDef.getId() + "]RedisDef.sentinels 不能为空");
        AssertUtil.assertNotBlank(sentinelRedisDef.getMasterName(), "[" + sentinelRedisDef.getId() + "]RedisDef.masterName 不能为空");
     ...
     * 应用默认的连接池配置
     *
     * @param sentinelRedisDef RedisDef 定义
     * @return 返回设置了默认连接池之后的配置
     */
    private Map<String, String> applyDefaultPoolConfig(SentinelRedisDef sentinelRedisDef) {

        Map<String, String> poolConfig = sentinelRedisDef.getPoolConfig();
        if (null == poolConfig) {
            poolConfig = new HashMap<>();
            sentinelRedisDef.setPoolConfig(poolConfig);
        }
    ...
    private String filterDefaultPassword(String password, String defVal) {
        return StringUtils.isBlank(password) ? defVal : password;
    }
