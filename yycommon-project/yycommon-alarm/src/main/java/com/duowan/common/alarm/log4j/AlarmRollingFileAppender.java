package com.duowan.common.alarm.log4j;
...
/**
 * @author Arvin
 * @since 2018/3/14 20:27
 */
@Plugin(name = AlarmRollingFileAppender.PLUGIN_NAME, category = "Core", elementType = Appender.ELEMENT_TYPE, printObject = true)
public class AlarmRollingFileAppender extends AbstractOutputStreamAppender<RollingFileManager> {

    public static final String PLUGIN_NAME = "AlarmRollingFile";
    ...
    private AlarmRollingFileAppender(final String name, final Layout<? extends Serializable> layout, final Filter filter,
    ...
    public void append(final LogEvent event) {
        ...
        // 这里指定告警操作, ERROR 级别以上的才会发送告警信息
        if (event.getLevel().isMoreSpecificThan(Level.ERROR)) {
            alarm(event);
        }
    }

    protected void alarm(LogEvent event) {
    ...
    public static <B extends AlarmRollingFileAppender.Builder<B>> AlarmRollingFileAppender createAppender(
        ...
        builder.withIgnoreExceptions(Booleans.parseBoolean(ignore, true));
        builder.withImmediateFlush(Booleans.parseBoolean(immediateFlush, true));
        builder.withLayout(layout);
        builder.withCreateOnDemand(false);
