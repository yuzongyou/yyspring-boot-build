...
public abstract class AbstractJdbc extends JdbcDaoSupport implements Jdbc {
    ...
    protected void logSqlAndParams(String sql, Object param) {
        if (log.isDebugEnabled()) {
            log.debug("{}, params={}", sql, JsonUtil.toJson(param));
        }
    }
            ...
            if (autoIncrementFieldDef != null) {
                ...
                GenerateKeyCallback callback = (rowIndex, primaryKey) -> {
                    try {
                        ...
                        throw new JdbcException("自增主键设置失败： " + e.getMessage(), e);
                    }
                };
